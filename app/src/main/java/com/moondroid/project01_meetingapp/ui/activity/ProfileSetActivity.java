...
public class ProfileSetActivity extends AppCompatActivity {
    ...
    public void loadBasicInfo() {
        ...
        if (GlobalInfo.myProfile.getUserGender() != null && GlobalInfo.myProfile.getUserGender().equals("여자")) {
            radioButtonMale.setChecked(false);
            radioButtonFemale.setChecked(true);
        }
        if (GlobalInfo.myProfile.getUserBirthDate() != null)
            tvBirthDate.setText(GlobalInfo.myProfile.getUserBirthDate());
        if (GlobalInfo.myProfile.getUserLocation() != null) {
            String[] locations = GlobalInfo.myProfile.getUserLocation().split(" ");
            tvLocation.setText(locations[0]);
        }
        if (GlobalInfo.myProfile.getUserProfileMessage() != null) {
            etMessage.setText(GlobalInfo.myProfile.getUserProfileMessage());
